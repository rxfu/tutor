...
@section('content')
<form action="{{ route('tutor.updateAdward', $items[0]->zjhm) }}" method="post" role="form" class="form-horizontal">
	{{ method_field('put') }}
	{{ csrf_field() }}
		        ...
		        <tbody>
		        	@for($i = 0; $i < 5; ++$i)
		        		<tr>
		                    ...
		                    <td>
		                    	<input type="text" name="item[{{ $i }}][sj]" id="item[{{ $i }}][sj]" class="form-control" placeholder="奖励名称、等级、时间" value="{{ isset($items[$i]) ? $items[$i]->sj : ''}}">
		                    </td>
		                    <td>

...
@section('subtitle')
编辑{{ $title }}“{{ $item->tutor->xm }}”信息
@stop

@section('content')
<form action="{{ route('tutor.updateSelection', $item->id) }}" method="post" role="form" class="form-horizontal">
	{{ method_field('put') }}
	{{ csrf_field() }}
	<fieldset>
		...
		<div class="form-group">
			...
			<div class="col-sm-10">
				<p class="form-control-static">{{ date('Y年m月', strtotime(substr($item->zjhm, 6, 8))) }}</p>
			</div>
		</div>
		<div class="form-group">
			...
			<div class="col-sm-10">
				...
				<select name="nsbxkzy" id="nsbxkzy" class="form-control">
					...
					@foreach ($subdisciplines->getAll() as $subdiscipline)
						{!! $subdiscipline->present()->option($item->nsbxkzy) !!}
					@endforeach
				</select>
			</div>
		</div>
		...
		<div class="form-group">
			...
			<div class="col-sm-10">
				...
				<label class="radio-inline">
					<input type="radio" name="sfjzds" value="0"{{ $item->sfjzds == '1' ? ' checked' : '' }}>&nbsp;否
				</label>
			</div>
		</div>
		<div class="form-group">
		...
		<div class="form-group">
			...
			<div class="col-sm-10">
				<textarea name="jl" cols="50" rows="10" class="form-control" placeholder="学习工作简历">{{ $item->jl }}</textarea>
			</div>
		</div>
		<div class="form-group">
			<label for="xzzdyjsqk" class="control-label col-sm-2">协助指导研究生情况</label>
			<div class="col-sm-10">
				<textarea name="xzzdyjsqk" cols="50" rows="10" class="form-control" placeholder="协助指导研究生情况">{{ $item->xzzdyjsqk }}</textarea>
			</div>
		</div>
		<div class="form-group">
			<label for="ksyjskc" class="control-label col-sm-2">开设研究生课程</label>
			<div class="col-sm-10">
				<textarea name="ksyjskc" cols="50" rows="10" class="form-control" placeholder="开设研究生课程">{{ $item->ksyjskc }}</textarea>
			</div>
		</div>
		<div class="form-group">
